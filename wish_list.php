...
// 読みたい本リストを【ログイン中のユーザー】に限定して取得
try {
  $dbh = getDbh(); // データベース接続を取得

  // SQL文: kadai_07から、ログイン中のuser_idに紐づく、statusが'read'の本だけを取得
  $sql = "SELECT * FROM kadai_07 WHERE user_id = :user_id AND status = 'wish' ORDER BY created_at DESC";
  $stmt = $dbh->prepare($sql);
  $stmt->bindValue(':user_id', $user_id, PDO::PARAM_INT); // ユーザーIDをバインド
  $stmt->execute();
  $wish_books = $stmt->fetchAll(PDO::FETCH_ASSOC); // データを取得
...
<body>
  ...
  <main>
    <div class="book_list container">
      <?php if (empty($wish_books)): ?>
        <p>まだ読みたい本は登録されていません</p>
      <?php else: ?>
        <?php foreach ($wish_books as $book): ?>
          <div class="book_card">
            <div class="book_img">
            ...
            <div class="book_details">
              ...
              <div class="read_date_info">
                <?php if ($book['read_date']): ?>
                  読んだ日 : <?= h($book['read_date']) ?>
                <?php endif; ?>
              </div>
              <div class="registration_actions_container">
                <div class="actions">
                  <!-- &：複数のクエリパラメータを区切るための記号 -->
                  <a href="edit.php?id=<?php echo h($book['id']); ?>&from=wish" class="edit-btn">編集する</a>
                  <a href="delete.php?id=<?php echo h($book['id']); ?>&from=wish"
                    class="delete-btn" onclick="return confirm('この本を削除しますか？');">削除する</a>

                  <a href="file_view.php?id=<?= h($book['id']); ?>&from=wish" class="upload-btn">画像アップロード</a>
                </div>
